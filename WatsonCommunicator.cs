...
public class WatsonCommunicator : MonoBehaviour
{
    ...
    void OnMessage(object resp, Dictionary<string, object> customData)
    {
        ...
        if (resp != null && (messageResponse.intents.Length > 0 || messageResponse.entities.Length > 0))
        {
            ...
            string[] outputresp = messageResponse.output.text;
            foreach(string a in outputresp)
            Debug.Log("Response: " +a);
            _testString = "<speak version=\"1.0\"><express-as type=\"GoodNews\">"+messageResponse.output.text[0]+"</express-as></speak>";
    ...
    private IEnumerator Examples()
    {
       ...
       // string wordToDelete = "goodbye";
        //if (!_textToSpeech.DeleteCustomizationWord(OnDeleteCustomizationWord, OnFailTTS, _createdCustomizationId, wordToDelete))
       ...
       // while (!_deleteCustomizationWordTested)
        ...
        //if (!_textToSpeech.DeleteCustomization(OnDeleteCustomization, OnFailTTS, _createdCustomizationId))
    ...
    private void OnFailTTS(RESTConnector.Error error, Dictionary<string, object> customData)
    {
